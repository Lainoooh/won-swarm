...
import {
  ...
  Database, Monitor, Key, ShieldAlert, RefreshCw
} from 'lucide-react';

// --- 专属 Logo 组件：蜂巢集群 (Hive Matrix) ---
...
const Modal = ({ title, onClose, children, footer, size = 'default', zIndex = 'z-50', heightClass = 'h-[88vh] max-h-[88vh]' }) => {
  ...
  return (
    <div className={`fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center ${zIndex} animate-in fade-in duration-200 px-4`}>
      <div className={`bg-white/95 backdrop-blur-2xl border border-white/60 rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 ${sizeClass} ${heightClass} cyber-modal`}>
        <div className="flex justify-between items-center p-2.5 border-b border-white/40 bg-white/40 shrink-0 cyber-modal-header">
        ...
        {footer && (
          <div className="p-2.5 border-t border-white/40 bg-white/40 flex justify-end gap-2 shrink-0 cyber-modal-footer">
            {footer}
          </div>
        )}
      </div>
...
const AgentManagement = () => {
  ...
  return (
    <div className="bg-white/70 backdrop-blur-2xl border border-white/60 rounded-xl flex flex-col flex-1 overflow-hidden shadow-sm m-1.5">
...
const SettingsView = ({ currentTheme, onThemeChange }) => {
  ...
  const handleTestConnection = () => {
    setIsTesting(true);
    setTestResult(null);
    setTimeout(() => { setIsTesting(false); setTestResult('success'); setTimeout(() => setTestResult(null), 3000); }, 1500);
  };
  ...
  return (
    <div className="bg-white/70 border border-white/60 rounded-xl flex flex-1 overflow-hidden shadow-sm m-1.5 relative z-10 backdrop-blur-2xl">
      <div className="w-56 bg-slate-50/50 border-r border-white/60 p-4 flex flex-col gap-2 shrink-0">
        ...
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-xs font-bold nav-btn ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20 settings-tab-active' : 'text-slate-600 hover:bg-white/60 border border-transparent hover:border-white/50 shadow-sm'}`}>
        ...
        <div className="max-w-3xl">
          {activeTab === 'models' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div><h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Cpu size={20} className="text-blue-600"/> 大模型驱动配置</h3><p className="text-xs text-slate-500 mt-1">配置全局 LLM 模型源，支持云端服务与本地私有化部署模型的混合调度。</p></div>
              <div className="bg-white/80 border border-blue-200/60 rounded-xl shadow-sm overflow-hidden relative backdrop-blur-sm">
                ...
                <div className="p-4 border-b border-white/60 flex justify-between items-center bg-blue-50/40">
                  <div className="flex items-center gap-2"><Database size={16} className="text-blue-600" /><span className="font-bold text-sm text-slate-800">本地私有化模型 (Local Model)</span></div>
                  <span className="px-2 py-0.5 bg-emerald-100/80 text-emerald-700 text-[10px] font-bold rounded border border-emerald-200/50">推荐方案</span>
                </div>
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-xs font-bold text-slate-600">服务框架 (Provider)</label><select className="w-full text-xs font-medium text-slate-800 bg-white/60 border border-white rounded-lg px-3 py-2 focus:border-blue-400 focus:outline-none shadow-sm"><option>Ollama (兼容 API)</option><option>vLLM</option><option>LM Studio</option><option>自定义 OpenAI 兼容协议</option></select></div>
                    <div className="space-y-1.5"><label className="text-xs font-bold text-slate-600">默认模型名称 (Model ID)</label><input type="text" defaultValue="llama3:8b" className="w-full text-xs font-mono text-slate-800 bg-white/60 border border-white rounded-lg px-3 py-2 focus:border-blue-400 focus:outline-none shadow-sm" /></div>
                  </div>
                  <div className="space-y-1.5"><label className="text-xs font-bold text-slate-600">接口地址 (Base URL)</label><input type="text" defaultValue="http://127.0.0.1:11434/v1" className="w-full text-xs font-mono text-slate-800 bg-white/60 border border-white rounded-lg px-3 py-2 focus:border-blue-400 focus:outline-none shadow-sm" /></div>
                  <div className="space-y-1.5 relative">
                    <label className="text-xs font-bold text-slate-600 flex justify-between"><span>访问密钥 (API Key)</span><span className="text-[10px] font-normal text-slate-400">若无需鉴权可留空或填 "ollama"</span></label>
                    <div className="relative"><Key size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input type="password" defaultValue="ollama" className="w-full text-xs font-mono text-slate-800 bg-white/60 border border-white rounded-lg pl-8 pr-3 py-2 focus:border-blue-400 focus:outline-none shadow-sm" /></div>
                  </div>
                  <div className="pt-2 flex items-center justify-between border-t border-white/60 mt-4">
                    <div className="text-xs font-medium h-6 flex items-center">
                      {isTesting && <span className="text-blue-500 flex items-center gap-1.5"><RefreshCw size={12} className="animate-spin"/> 正在与本地端口握手...</span>}
                      {testResult === 'success' && <span className="text-emerald-600 flex items-center gap-1.5"><CheckCircle2 size={12}/> 连接成功！模型响应正常。</span>}
                    </div>
                    <div className="flex gap-2">
                      <button onClick={handleTestConnection} disabled={isTesting} className="px-4 py-2 text-xs font-bold text-slate-600 bg-white/50 hover:bg-white rounded-lg transition-colors border border-white shadow-sm disabled:opacity-50">测试连接</button>
                      <button className="px-4 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors shadow-sm flex items-center gap-1.5"><Save size={14}/> 保存配置</button>
                    </div>
                  </div>
                </div>
              </div>
              <div className="bg-white/40 border border-white/60 rounded-xl p-4 flex items-center justify-between opacity-60 grayscale hover:grayscale-0 transition-all cursor-pointer backdrop-blur-sm shadow-sm">
                <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-white/80 border border-white flex items-center justify-center shadow-sm"><Server size={14} className="text-slate-500"/></div><div><div className="text-sm font-bold text-slate-700">OpenAI / Anthropic 云端接入</div><div className="text-[10px] text-slate-400">点击配置云端商业大模型...</div></div></div><ChevronRight size={16} className="text-slate-300"/>
              </div>
            </div>
          )}
          ...
          {activeTab === 'appearance' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div>
                <h3 className="text-lg font-black text-slate-800 flex items-center gap-2"><Monitor size={20} className="text-blue-600"/> UI 主题与外观</h3>
                <p className="text-xs text-slate-500 mt-1">全局切换系统视觉体验，支持清透风格与赛博朋克深色模式。</p>
              </div>

              <div className="grid grid-cols-2 gap-5 mt-4">
                {/* 蔚蓝晶透 主题卡片 - 采用绝对 Hex 值防止被全局深色模式 CSS 强行覆盖 */}
                <div
                  onClick={() => onThemeChange('azure')}
                  className={`border-2 rounded-xl p-1 cursor-pointer transition-all duration-300 shadow-sm ${currentTheme === 'azure' ? 'border-[#3b82f6] shadow-md scale-[1.02] bg-[#eff6ff]' : 'border-[#cbd5e1] hover:border-[#93c5fd] bg-[#f8fafc]'}`}
                >
                  <div className="h-32 rounded-lg overflow-hidden bg-gradient-to-br from-[#f8fafc] to-[#e8eef3] border border-[#f1f5f9] p-3 relative flex flex-col justify-between">
                    ...
                    <div className="w-full h-12 bg-[#ffffff] rounded shadow-sm border border-[#ffffff]/50 flex items-center justify-center">
                      <span className="text-[#3b82f6] font-bold text-[10px] px-2 py-1 bg-[#eff6ff] rounded">Active Component</span>
                    </div>
                    {/* 增加 z-50 确保打勾图标在最上层 */}
                    {currentTheme === 'azure' && <div className="absolute top-2 right-2 bg-[#3b82f6] text-[#ffffff] p-1 rounded-full shadow-sm z-50"><Check size={12} strokeWidth={3}/></div>}
                  </div>
                  <div className="p-3 text-center">
                    <div className="text-sm font-black text-[#1e293b]">蔚蓝晶透</div>
                    <div className="text-[10px] text-[#64748b] mt-0.5">Azure Glass - 默认浅色高斯模糊</div>
                  </div>
                </div>

                {/* 霓虹深渊 主题卡片 - 同样采用绝对 Hex 值，使其在浅色模式下也能保持深色预览外观 */}
                <div
                  onClick={() => onThemeChange('cyberpunk')}
                  className={`border-2 rounded-xl p-1 cursor-pointer transition-all duration-300 shadow-sm ${currentTheme === 'cyberpunk' ? 'border-[#22d3ee] shadow-[0_0_15px_rgba(6,182,212,0.4)] scale-[1.02] bg-[#0f172a]' : 'border-[#334155] hover:border-[#475569] bg-[#1e293b]'}`}
                >
                  <div className="h-32 rounded-lg overflow-hidden bg-gradient-to-br from-[#020617] to-[#0f172a] border border-[#1e293b] p-3 relative flex flex-col justify-between">
                    ...
                    <div className="w-full h-12 bg-[#1e293b] rounded border border-[#334155] flex items-center justify-center relative z-10">
                      <span className="text-[#22d3ee] font-bold text-[10px] px-2 py-1 bg-[#083344]/50 border border-[#155e75] rounded drop-shadow-[0_0_5px_rgba(6,182,212,0.8)]">Neon Cyber Component</span>
                    </div>
                    {/* 增加 z-50 确保打勾图标在最上层 */}
                    {currentTheme === 'cyberpunk' && <div className="absolute top-2 right-2 bg-[#06b6d4] text-[#0f172a] p-1 rounded-full shadow-[0_0_10px_rgba(6,182,212,0.8)] z-50"><Check size={12} strokeWidth={3}/></div>}
                  </div>
                  <div className="p-3 text-center">
                    <div className="text-sm font-black text-[#f8fafc]">霓虹深渊</div>
                    <div className="text-[10px] text-[#94a3b8] mt-0.5">Neon Cyberpunk - 朋克科技护眼模式</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'security' && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div><h3 className="text-lg font-black text-rose-600 flex items-center gap-2"><ShieldAlert size={20} /> 智能体行为与安全边界</h3><p className="text-xs text-slate-500 mt-1">防止 Agent 意外修改系统或造成天价账单，建议在生产环境中收紧权限。</p></div>
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl shadow-sm">
                  <div><div className="text-sm font-bold text-slate-800">允许执行宿主机终端命令 (Terminal)</div><div className="text-[10px] text-slate-400 mt-1">关闭后，Agent 生成的 shell 脚本将需要人工点击批准后才能运行。</div></div>
                  <div className="w-10 h-6 bg-rose-500 rounded-full flex items-center p-1 cursor-pointer shadow-inner justify-end"><div className="w-4 h-4 bg-white rounded-full shadow-sm"></div></div>
                </div>
                <div className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl shadow-sm">
                  <div><div className="text-sm font-bold text-slate-800">沙盒文件系统隔离</div><div className="text-[10px] text-slate-400 mt-1">开启后，Agent 只能在工作区挂载目录内读写，无法访问系统根目录。</div></div>
                  <div className="w-10 h-6 bg-emerald-500 rounded-full flex items-center p-1 cursor-pointer shadow-inner justify-end"><div className="w-4 h-4 bg-white rounded-full shadow-sm"></div></div>
                </div>
                <div className="flex items-center justify-between p-4 bg-white/80 backdrop-blur-sm border border-white/60 rounded-xl shadow-sm">
                  <div className="flex-1"><div className="text-sm font-bold text-slate-800">全局 Token 熔断阈值 (单日)</div><div className="text-[10px] text-slate-400 mt-1">当 Swarm 集群消耗的 tokens 达到此数值，将自动暂停所有排队任务。</div></div>
                  <div className="w-40 relative"><input type="number" defaultValue="1000000" className="w-full text-sm font-mono text-slate-800 bg-white/60 border border-white rounded-lg pr-12 pl-3 py-2 focus:border-blue-400 focus:outline-none shadow-sm text-right" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 font-bold">TOK</span></div>
                </div>
              </div>
            </div>
          )}
          ...
          {activeTab === 'secrets' && (
            <div className="flex flex-col items-center justify-center h-64 text-slate-400 space-y-3 animate-in fade-in bg-white/40 border border-white/60 rounded-xl shadow-sm">
              <Key size={48} className="opacity-20" /><div className="text-sm font-bold">全局凭证库开发中...</div><div className="text-xs max-w-sm text-center">这里未来将配置数据库密码、Github Token、第三方 API Key 等，并通过变量形式安全下发给需要的 Agent，杜绝硬编码。</div>
            </div>
          )}
        </div>
      </div>
...
export default function App() {
  const [currentRoute, setCurrentRoute] = useState('settings'); // 默认进入设置页以体验新功能
  const [activeProject, setActiveProject] = useState(null);
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState(false);
  const [theme, setTheme] = useState('azure'); // 新增双轨主题状态

  // 动态注入 Favicon
  useEffect(() => {
    document.title = "OneSwarm | 智能体协作平台";
    const strokeColor = theme === 'cyberpunk' ? '#06b6d4' : '#3b82f6';
    const fillColor = theme === 'cyberpunk' ? '#22d3ee' : '#0ea5e9';
  ...
  return (
    // 主容器：原版 bg-[#e8eef3] 完全保留！
    <div className={`min-h-screen font-sans flex overflow-hidden relative ${theme === 'cyberpunk' ? 'theme-cyberpunk selection:bg-cyan-900 selection:text-cyan-200' : 'bg-[#e8eef3] text-slate-800 selection:bg-cyan-200 selection:text-cyan-900'}`}>

      {/* 赛博深色 CSS 强力覆盖隔离层：不影响浅色结构 */}
      <style dangerouslySetInnerHTML={{__html: `
        .theme-cyberpunk { background-color: #020617; color: #e2e8f0; }
        .theme-cyberpunk .bg-white\\/40, .theme-cyberpunk .bg-white\\/70, .theme-cyberpunk .bg-white\\/80, .theme-cyberpunk .bg-white\\/30, .theme-cyberpunk .bg-white\\/60, .theme-cyberpunk .bg-white\\/50, .theme-cyberpunk .bg-white\\/20 { background-color: rgba(15, 23, 42, 0.75) !important; border-color: rgba(6, 182, 212, 0.2) !important; }
        .theme-cyberpunk .bg-white\\/90, .theme-cyberpunk .bg-white\\/95 { background-color: rgba(15, 23, 42, 0.95) !important; border-color: rgba(6, 182, 212, 0.3) !important; }
        .theme-cyberpunk .bg-slate-50\\/50, .theme-cyberpunk .bg-slate-50\\/30 { background-color: rgba(30, 41, 59, 0.6) !important; }
        .theme-cyberpunk .bg-white { background-color: #0f172a !important; border-color: #334155 !important; color: #f8fafc !important; }
        .theme-cyberpunk .bg-slate-50 { background-color: #1e293b !important; border-color: #334155 !important; }
        .theme-cyberpunk .bg-slate-100 { background-color: #334155 !important; border-color: #475569 !important; color: #f8fafc !important; }
        .theme-cyberpunk .text-slate-800, .theme-cyberpunk .text-slate-700 { color: #f8fafc !important; }
        .theme-cyberpunk .text-slate-600, .theme-cyberpunk .text-slate-500 { color: #94a3b8 !important; }
        .theme-cyberpunk .text-slate-400 { color: #64748b !important; }
        .theme-cyberpunk .text-blue-600, .theme-cyberpunk .text-blue-700 { color: #06b6d4 !important; }
        .theme-cyberpunk .bg-blue-600 { background-color: #0891b2 !important; box-shadow: 0 0 12px rgba(6,182,212,0.6) !important; }
        .theme-cyberpunk .hover\\:bg-blue-700:hover { background-color: #0e7490 !important; }
        .theme-cyberpunk .bg-blue-50, .theme-cyberpunk .bg-blue-50\\/80, .theme-cyberpunk .bg-blue-50\\/50 { background-color: rgba(6, 182, 212, 0.1) !important; border-color: rgba(6, 182, 212, 0.3) !important; color: #22d3ee !important; }
        .theme-cyberpunk .border-blue-200, .theme-cyberpunk .border-blue-100 { border-color: #164e63 !important; }
        .theme-cyberpunk .border-slate-200, .theme-cyberpunk .border-slate-100, .theme-cyberpunk .border-slate-300 { border-color: #334155 !important; }
        .theme-cyberpunk .border-white\\/60, .theme-cyberpunk .border-white\\/40, .theme-cyberpunk .border-white\\/80, .theme-cyberpunk .border-white { border-color: rgba(6,182,212,0.2) !important; }
        .theme-cyberpunk input, .theme-cyberpunk select, .theme-cyberpunk textarea { background-color: #0f172a !important; border-color: #334155 !important; color: #22d3ee !important; }
        .theme-cyberpunk input:focus, .theme-cyberpunk select:focus, .theme-cyberpunk textarea:focus { border-color: #06b6d4 !important; box-shadow: 0 0 0 1px #06b6d4 !important; }
        .theme-cyberpunk svg polygon[stroke="#3b82f6"] { stroke: #06b6d4 !important; filter: drop-shadow(0 0 4px rgba(6,182,212,0.8)); }
        .theme-cyberpunk svg circle[fill="#0ea5e9"] { fill: #22d3ee !important; filter: drop-shadow(0 0 6px rgba(34,211,238,0.8)); }
        .theme-cyberpunk svg circle[fill="#38bdf8"], .theme-cyberpunk svg circle[fill="#3b82f6"] { fill: #67e8f9 !important; }

        /* 1. 强化整体结构与弹窗骨架的光晕边界 */
        .theme-cyberpunk aside.cyber-aside {
          border-right: 1px solid rgba(6, 182, 212, 0.6) !important;
          box-shadow: 2px 0 15px rgba(6, 182, 212, 0.2) !important;
        }
        .theme-cyberpunk header.cyber-header {
          border-bottom: 1px solid rgba(6, 182, 212, 0.6) !important;
          box-shadow: 0 2px 15px rgba(6, 182, 212, 0.2) !important;
        }
        .theme-cyberpunk .cyber-modal {
          border: 1px solid #06b6d4 !important;
          box-shadow: 0 0 40px rgba(6, 182, 212, 0.4), inset 0 0 20px rgba(6, 182, 212, 0.15) !important;
        }
        .theme-cyberpunk .cyber-modal-header {
          border-bottom: 1px solid rgba(6, 182, 212, 0.6) !important;
          background: rgba(6, 182, 212, 0.1) !important;
        }
        .theme-cyberpunk .cyber-modal-footer {
          border-top: 1px solid rgba(6, 182, 212, 0.6) !important;
          background: rgba(6, 182, 212, 0.05) !important;
        }

        /* 2. 导航栏未选中 Hover 效果强化 */
        .theme-cyberpunk .nav-btn:hover {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.2) 0%, transparent 100%) !important;
          color: #22d3ee !important;
          text-shadow: 0 0 8px rgba(34, 211, 238, 0.5);
          box-shadow: inset 2px 0 0 0 #06b6d4 !important;
        }

        /* 3. 导航栏已选中 Active 效果强化 */
        .theme-cyberpunk .nav-active {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.3) 0%, rgba(6, 182, 212, 0.05) 100%) !important;
          border-left: 3px solid #06b6d4 !important;
          border-radius: 0 6px 6px 0 !important;
          color: #67e8f9 !important;
          text-shadow: 0 0 10px rgba(34, 211, 238, 0.8);
          box-shadow: inset 4px 0 0 0 #06b6d4, inset 0 0 15px rgba(6, 182, 212, 0.2) !important;
        }
        .theme-cyberpunk .settings-tab-active {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.3) 0%, rgba(6, 182, 212, 0.05) 100%) !important;
        ...
        /* 4. 全局灰色分割线转霓虹色 */
        .theme-cyberpunk .border-slate-200,
        .theme-cyberpunk .border-slate-200\\/60,
        .theme-cyberpunk .border-slate-200\\/50,
        .theme-cyberpunk .border-slate-300,
        .theme-cyberpunk .border-slate-300\\/40,
        .theme-cyberpunk .border-slate-100 {
           border-color: rgba(6, 182, 212, 0.3) !important;
        }
        .theme-cyberpunk .bg-slate-300,
        .theme-cyberpunk .bg-slate-200,
        .theme-cyberpunk .bg-slate-200\\/60,
        .theme-cyberpunk .bg-slate-200\\/50 {
           background-color: rgba(6, 182, 212, 0.3) !important;
        }

        /* 5. 内部面板通用边框 */
        .theme-cyberpunk .border-white\\/60, .theme-cyberpunk .border-white\\/40, .theme-cyberpunk .border-white\\/80, .theme-cyberpunk .border-white {
          border-color: rgba(6,182,212,0.5) !important;
          box-shadow: inset 0 0 15px rgba(6,182,212,0.1), 0 0 10px rgba(6,182,212,0.15) !important;
        }

        /* ===== 高级 Hover 与选中态 (全局背景点亮 + 炫酷科技光晕) ===== */

        .theme-cyberpunk tr.hover\\:bg-white\\/60:hover {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.2) 0%, rgba(6, 182, 212, 0.05) 100%) !important;
        }
        .theme-cyberpunk tr.hover\\:bg-white\\/60:hover td:first-child {
          box-shadow: inset 3px 0 0 0 #06b6d4, inset 0 0 15px rgba(6,182,212,0.1) !important;
        }
        .theme-cyberpunk tr.hover\\:bg-white\\/60:hover .text-slate-800,
        .theme-cyberpunk tr.hover\\:bg-white\\/60:hover .text-blue-600 {
          color: #22d3ee !important;
          text-shadow: 0 0 10px rgba(34, 211, 238, 0.6);
        }
        .theme-cyberpunk tr.hover\\:bg-white\\/60:hover .text-slate-400 {
          color: #a5f3fc !important;
        }

        .theme-cyberpunk .hover\\:bg-blue-50\\/80:hover {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.25) 0%, rgba(6, 182, 212, 0.05) 100%) !important;
          box-shadow: inset 2px 0 0 0 #06b6d4, inset 0 0 15px rgba(6,182,212,0.15) !important;
        }
        .theme-cyberpunk .hover\\:bg-blue-50\\/80:hover .text-slate-800,
        .theme-cyberpunk .hover\\:bg-blue-50\\/80:hover .text-slate-700 {
          color: #22d3ee !important;
          text-shadow: 0 0 8px rgba(34, 211, 238, 0.5);
        }

        .theme-cyberpunk .bg-blue-50\\/80 {
          background: linear-gradient(90deg, rgba(6, 182, 212, 0.3) 0%, rgba(6, 182, 212, 0.1) 100%) !important;
          border-color: rgba(6, 182, 212, 0.5) !important;
          box-shadow: inset 4px 0 0 0 #06b6d4, inset 0 0 20px rgba(6, 182, 212, 0.2) !important;
        }
        .theme-cyberpunk .bg-blue-50\\/80 .text-slate-800,
        .theme-cyberpunk .bg-blue-50\\/80 .text-slate-700,
        .theme-cyberpunk .bg-blue-50\\/80 .text-slate-600,
        .theme-cyberpunk .bg-blue-50\\/80 .text-blue-700,
        .theme-cyberpunk .bg-blue-50\\/80 .truncate {
          color: #67e8f9 !important;
          text-shadow: 0 0 10px rgba(34, 211, 238, 0.6);
        }
        .theme-cyberpunk .bg-blue-50\\/80 .bg-blue-500 {
          background-color: #06b6d4 !important;
          box-shadow: 0 0 10px rgba(6, 182, 212, 0.8) !important;
        }

        .theme-cyberpunk .hover\\:border-blue-300:hover {
          background-color: rgba(6, 182, 212, 0.15) !important;
          border-color: rgba(6, 182, 212, 0.8) !important;
          box-shadow: 0 0 25px rgba(6, 182, 212, 0.3), inset 0 0 20px rgba(6, 182, 212, 0.2) !important;
        }
        .theme-cyberpunk .hover\\:border-blue-300:hover .text-slate-800 {
          color: #22d3ee !important;
          text-shadow: 0 0 8px rgba(34, 211, 238, 0.4);
        }

        .theme-cyberpunk .hover\\:border-blue-200:hover {
          background-color: rgba(6, 182, 212, 0.15) !important;
          border-color: rgba(6, 182, 212, 0.6) !important;
          box-shadow: 0 0 15px rgba(6, 182, 212, 0.3), inset 0 0 15px rgba(6, 182, 212, 0.15) !important;
        }
        .theme-cyberpunk .hover\\:border-blue-200:hover .text-slate-800 {
          color: #22d3ee !important;
          text-shadow: 0 0 6px rgba(34, 211, 238, 0.3);
        }
      `}} />

      {/* 底层深浅两套背景完全隔离：蔚蓝晶透就是您的原版代码 */}
      ...
      <aside className={`${isSidebarCollapsed ? 'w-16' : 'w-48'} border-r border-white/60 bg-white/40 backdrop-blur-2xl flex flex-col z-30 shadow-sm transition-all duration-300 relative shrink-0 cyber-aside`}>
        <div className={`h-10 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'px-3'} border-b border-white/40 relative shrink-0`}>
          <div className="flex items-center gap-1.5 text-blue-600 overflow-hidden">
            {/* 更新为最新的蜂巢 Logo */}
            <HiveMatrixLogo className="w-5 h-5 drop-shadow-sm shrink-0" />
            {!isSidebarCollapsed && <span className={`font-black text-sm tracking-tight whitespace-nowrap ${theme === 'cyberpunk' ? 'text-slate-100' : 'text-slate-800'}`}>One<span className="text-blue-600">Swarm</span></span>}
          </div>
        ...
        <div className={`py-2 flex-1 ${isSidebarCollapsed ? 'px-1.5' : 'px-2'}`}>
          <nav className="space-y-0.5">
            {navItems.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => handleNavClick(item.id)} title={isSidebarCollapsed ? item.label : ''} className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center px-0' : 'gap-2 px-2'} py-1.5 rounded-md transition-all duration-200 text-xs font-bold nav-btn ${currentRoute === item.id ? 'bg-white/80 text-blue-700 shadow-sm nav-active' : (theme === 'cyberpunk' ? 'text-slate-400 hover:bg-white/20' : 'text-slate-500 hover:bg-white/50')}`}>
          ...
          <div className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : 'gap-2'}`}>
            ...
            {!isSidebarCollapsed && (
              <div className="truncate leading-tight">
                <div className={`text-[11px] font-bold truncate ${theme === 'cyberpunk' ? 'text-slate-200' : 'text-slate-800'}`}>Admin</div>
      ...
      <main className="flex-1 flex flex-col min-w-0 relative z-10 h-screen overflow-hidden">
        <header className="h-10 border-b border-white/40 bg-white/40 backdrop-blur-2xl flex items-center justify-between px-3 z-20 shrink-0 cyber-header">
          <div className={`flex items-center text-[11px] font-medium bg-white/60 px-2 py-0.5 rounded border border-white shadow-sm ${theme === 'cyberpunk' ? 'text-cyan-400' : 'text-slate-500'}`}>
            <span>OneSwarm</span><ChevronRight size={10} className="mx-1 opacity-50" />
            {currentRoute === 'projects' && activeProject ? (
              <><span className="cursor-pointer hover:text-blue-600" onClick={() => setActiveProject(null)}>项目管理</span><ChevronRight size={10} className="mx-1 opacity-50" /><span className="text-blue-600 font-bold truncate max-w-[150px]">{activeProject.name}</span></>
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="text-slate-400 hover:text-blue-500 relative transition-colors bg-white/60 p-1 rounded-full border border-white shadow-sm">
        ...
        <div className="p-1.5 flex-1 overflow-hidden flex flex-col relative z-10">
          ...
          {/* 新增独立设置页面路由 */}
          {currentRoute === 'settings' && <SettingsView currentTheme={theme} onThemeChange={setTheme} />}
        </div>
      </main>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* 注入深色模式专属的滚动条 */
        .theme-cyberpunk .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(71, 85, 105, 0.8); }
        .theme-cyberpunk .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(6, 182, 212, 0.6); }
      `}} />
    </div>
  );
}