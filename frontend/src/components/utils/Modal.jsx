...
// 后端 req_type 合法值：'new' | 'change' | 'bug'
// 支持数字枚举值 (1=new, 2=change, 3=bug) 或字符串值直接传递
export const ReqTypeSelect = ({ value, onChange, placeholder = '选择需求类型', disabled = false, size = 'default', useEnum = false }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [dropdownDirection, setDropdownDirection] = useState('down');
  const triggerRef = React.useRef(null);

  // 枚举值映射：1=new, 2=change, 3=bug
  const options = [
    { value: 'new', enumValue: 1, label: '新增需求', desc: '新功能或新需求' },
    { value: 'change', enumValue: 2, label: '变更需求', desc: '需求变更或修改' },
  ...
  const handleSelect = (opt) => {
    onChange(useEnum ? opt.enumValue : opt.value);
    setIsOpen(false);
  };
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          <div className={`absolute left-0 w-full bg-white border border-slate-200 rounded-lg shadow-xl z-[100] overflow-hidden ${
            ...
            {options.map(opt => (
              <div
                ...
                className={`px-3 py-2 hover:bg-blue-50 cursor-pointer transition-colors border-b border-slate-100 last:border-0 ${value === (useEnum ? opt.enumValue : opt.value) ? 'bg-blue-50/50' : ''}`}
              >
...
// 支持数字枚举值 (1=P0, 2=P1, 3=P2, 4=P3) 或字符串值直接传递
export const PrioritySelect = ({ value, onChange, placeholder = '选择优先级', disabled = false, size = 'default', useEnum = false }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [dropdownDirection, setDropdownDirection] = useState('down');
  const triggerRef = React.useRef(null);

  // 枚举值映射：1=P0, 2=P1, 3=P2, 4=P3
  const options = [
    { value: 'P0', enumValue: 1, label: 'P0', desc: '最高优先级', color: 'text-rose-600 bg-rose-50' },
    { value: 'P1', enumValue: 2, label: 'P1', desc: '高优先级', color: 'text-amber-600 bg-amber-50' },
    { value: 'P2', enumValue: 3, label: 'P2', desc: '普通优先级', color: 'text-blue-600 bg-blue-50' },
...
export const CreateRequirementModal = ({ onClose, onSave, agents = [] }) => {
  ...
  const [features, setFeatures] = useState([
    { id: 1, name: '多端自适应布局', priority: 2, reqType: 1, desc: '支持在移动端和 PC 端无缝切换，保证交互体验一致。', isEditing: false },
    { id: 2, name: '数据看板图表展示', priority: 1, reqType: 1, desc: '集成 ECharts，提供基础的折线图、柱状图数据渲染。', isEditing: false }
  ]);
  ...
  const handleCreateRequirement = async () => {
    ...
    try {
      // 优先级枚举映射：1->P0, 2->P1, 3->P2, 4->P3
      const priorityMap = { 1: 'P0', 2: 'P1', 3: 'P2', 4: 'P3' };
                  ...
                  <div className="flex items-start justify-between gap-4">
                     ...
                     <div className="flex items-center gap-1.5 shrink-0">
                       {f.isEditing ? (
                         ...
                         <>
                           <span className={`text-[9px] px-1 py-0.5 rounded border font-mono font-bold ${f.reqType === 1 ? 'bg-green-50 text-green-600 border-green-200' : f.reqType === 2 ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                             {f.reqType === 1 ? '新增' : f.reqType === 2 ? '变更' : '缺陷'}
                           </span>
                           <span className={`text-[9px] px-1 py-0.5 rounded border font-mono font-bold ${f.priority === 1 ? 'bg-red-50 text-red-600 border-red-200' : f.priority === 2 ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-blue-50 text-blue-600 border-blue-200'}`}>
                             {f.priority === 1 ? 'P0' : f.priority === 2 ? 'P1' : f.priority === 3 ? 'P2' : 'P3'}
                           </span>
                         </>
                       )}
                     </div>
