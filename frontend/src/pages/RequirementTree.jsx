  ...
  return (
    <div className="flex flex-1 w-full animate-in slide-in-from-right-4 duration-300 bg-white/70 backdrop-blur-2xl border border-white/60 rounded-xl shadow-sm overflow-hidden min-h-0">
      ...
      <div className="w-48 shrink-0 flex flex-col border-r border-slate-300/40 bg-white/20">
        <div className="p-3 border-b border-slate-200/60 bg-white/40 flex flex-col gap-2 shrink-0">
          <div className="flex items-center gap-1.5 text-slate-800">
            <FolderOpen size={14} className="text-blue-600"/>
            <h3 className="font-bold text-xs">项目列表</h3>
          </div>
          <div className="relative">
            <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="搜索项目..." className="bg-white border border-slate-200 text-slate-700 text-xs rounded-lg pl-8 pr-3 py-2 focus:outline-none focus:border-blue-400 w-full shadow-sm" />
          </div>
        </div>
          ...
          {projects.map(p => {
            ...
            return (
              <div
                ...
                className={`px-2.5 py-2 rounded-lg cursor-pointer transition-all border text-xs font-bold flex items-center justify-between group relative overflow-hidden
              ...
              >
                 ...
                 {isActive && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shadow-[0_0_6px_rgba(59,130,246,0.8)] flex-shrink-0 animate-pulse z-10"></div>}
              </div>
            );
          })}
        </div>

        <div className="p-2.5 border-t border-slate-200/60 bg-white/40 flex justify-between items-center text-xs text-slate-600 shrink-0">
           <span>共 {projects.length} 项</span>
           <div className="flex items-center gap-0.5">
             <button className="p-1 hover:bg-white rounded-lg"><ChevronDown size={12} className="rotate-90"/></button>
      ...
      <div className="flex-1 flex flex-col bg-transparent min-w-0">
        <div className="p-3 border-b border-slate-200/60 bg-white/40 flex justify-between items-center shrink-0">
          <div className="flex flex-col">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {currentProject && (
              <div className="flex flex-wrap items-center gap-2 ml-1 bg-white border border-slate-200 px-2.5 py-1.5 rounded-lg shadow-sm text-xs font-medium text-slate-600">
                 <span className="flex items-center gap-1"><Cpu size={12} className="text-blue-500"/> {currentProject.manager_name}</span>
                 <div className="w-px h-3 bg-slate-200"></div>
                 <span className="flex items-center gap-1"><Calendar size={12} className="text-slate-400"/> {currentProject.start_date} 至 {currentProject.end_date}</span>
                 <div className="w-px h-3 bg-slate-200"></div>
                 ...
                 <span className="flex items-center gap-1">
                   进度:
                   <div className="w-12 h-1.5 bg-slate-200 rounded-full overflow-hidden shadow-inner">
          ...
          <button onClick={() => setShowCreateReqModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 shadow-sm shrink-0 transition-colors">
            <Plus size={14} strokeWidth={2.5} /> 新增需求大纲
          </button>
        </div>
        ...
        <div className="flex-1 p-2 lg:p-3 overflow-auto custom-scrollbar bg-transparent">
          <div className="max-w-5xl mx-auto bg-white/80 backdrop-blur-md border border-white/60 rounded-xl shadow-sm p-1 flex flex-col h-full">
            ...
            {/* 表头布局 */}
            <div className="flex items-center px-3 py-2 border-b border-slate-200/60 bg-white/50 rounded-t-lg shrink-0">
              <div className="flex-1 flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider min-w-[200px]">
                <input type="checkbox" className="w-3.5 h-3.5 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer" title="全选/取消全选" />
                <span>大纲层级</span>
              </div>
              <div className="w-20 text-xs font-bold text-slate-500 uppercase text-center shrink-0">优先级</div>
              <div className="w-20 text-xs font-bold text-slate-500 uppercase text-center shrink-0">状态</div>
              <div className="w-[280px] text-xs font-bold text-slate-500 uppercase text-center border-l border-slate-200/60 pl-2 shrink-0">子级配置</div>
              <div className="w-[160px] text-xs font-bold text-slate-500 uppercase text-right border-l border-slate-200/60 pl-2 pr-4 shrink-0">当前节点控制</div>
            </div>
